...
unsigned int* extract_response_codes_dtls12(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    (memcmp(&buf[byte_count+1], dtls12_version, 2) == 0)) {
      ...
      if (buf_size - byte_count - 13 - record_length < 0 || record_length==0) {
        record_length=buf_size-byte_count;
...
static void resp_append_code(unsigned int code, unsigned int **state_sequence_ref, unsigned int *state_count_ref) {
  ...
  *state_sequence_ref = (unsigned int *)ck_realloc(*state_sequence_ref, *state_count_ref * sizeof(unsigned int));
  ...
  (*state_sequence_ref)[*state_count_ref - 1] = code;
}
...
int net_send(int sockfd, struct timeval timeout, char *mem, unsigned int len, int disable_shmsync) {
  unsigned int byte_count = 0;
  int n;
  struct pollfd pfd[1];
  pfd[0].fd = sockfd;
  ...
  if(!disable_shmsync && !session_msg->is_end)
    shm_notify(&session_msg->state,SERVER);
  return byte_count;
}

int net_recv(int sockfd, struct timeval timeout, int poll_w, char **response_buf, unsigned int *len, int disable_shmsync) {
  char temp_buf[1000];
  int n;
  struct pollfd pfd[1];
  pfd[0].fd = sockfd;
  ...
  int rv;
  if(!disable_shmsync){
    shm_wait(&session_msg->state,SERVER,-1);
    rv=poll(pfd, 1, 0);
  }
  else
    rv=poll(pfd, 1, poll_w);
