...
static s32 shm_id,shm_msg_id,shm_netport_id,shm_check_id;         /* ID of the SHM region             */
...
int bogus_crash_check = 0;
int disable_shm = 0;
int disable_shmsync = 0;
int *is_M2;
...
unsigned int* get_state_sequence(int end_idx, unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref){
  unsigned int *result, *suffix_state_sequence, *prefix_state_sequence=NULL, prefix_state_count=0;
  if(disable_shm)
    return (*extract_response_codes)(buf,buf_size,state_count_ref);

  *state_count_ref=0;
  end_idx=end_idx<M2_start_region_ID?end_idx:M2_start_region_ID-1;
  if(end_idx>=0){
    prefix_state_sequence=queue_cur->regions[end_idx].state_sequence;
    prefix_state_count=queue_cur->regions[end_idx].state_count;
  }
  
  if(buf_size>0){
    suffix_state_sequence=(*extract_response_codes)(buf,buf_size,state_count_ref);
    result=(unsigned int *)ck_alloc((prefix_state_count+*state_count_ref)*sizeof(unsigned int));
    memcpy(result, prefix_state_sequence, prefix_state_count*sizeof(unsigned int));
    memcpy(&result[prefix_state_count], suffix_state_sequence, *state_count_ref*sizeof(unsigned int));
    ck_free(suffix_state_sequence);
  }
  else{
    result=(unsigned int *)ck_alloc(prefix_state_count*sizeof(unsigned int));
    memcpy(result, prefix_state_sequence, prefix_state_count*sizeof(unsigned int));
  }
  *state_count_ref+=prefix_state_count;
  return result;
}
...
void update_region_annotations(struct queue_entry* q)
{
  ...
  for (i = 0; i < messages_sent; i++) {
    if ((response_bytes[i] == 0) || ( i > 0 && (response_bytes[i] - response_bytes[i - 1] == 0))) {
      ...
      q->regions[i].state_sequence = get_state_sequence(i, response_buf, response_bytes[i], &state_count);
...
void update_fuzzs() {
  unsigned int state_count, state_count2, i, discard;
  unsigned int *state_sequence = get_state_sequence(messages_sent-1, response_buf, response_buf_size, &state_count),*state_sequence2;
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  unsigned int *state_sequence = get_state_sequence(messages_sent-1, response_buf, response_buf_size, &state_count);
...
static int shm_read(int sockfd,char **response_buf,int *response_buf_size){
  int remain,result;

  if((result=shm_wait(&session_msg->state,SERVER,waiting_time)))
    return 1;
  if(!*is_M2)
    return 0;
  if(session_msg->len2==0)
    return -1;
  
  *response_buf=(char *)ck_realloc(*response_buf,*response_buf_size+session_msg->len2);
  
  for(remain=session_msg->len2;remain>0;){
    if(remain<MAX_MSG_SIZE){
      memcpy(*response_buf+*response_buf_size,session_msg->data2,remain);
      *response_buf_size+=remain;
      remain=0;
      session_msg->len2=0;
    }
    else{
      memcpy(*response_buf+*response_buf_size,session_msg->data2,MAX_MSG_SIZE);
      remain-=MAX_MSG_SIZE;
      session_msg->len2-=MAX_MSG_SIZE;
      *response_buf_size+=MAX_MSG_SIZE;
      shm_notify(&session_msg->state,SERVER);
      shm_wait(&session_msg->state,SERVER,waiting_time);
    }
  }
...
static int shm_write(int sockfd, char *mem, unsigned int len){
  int remain,p;
  
  shm_wait(&session_msg->state,SERVER,waiting_time);
  if(errno==ETIMEDOUT)
    return 0;
  if(session_msg->is_end){
    return -1;
  }

  session_msg->len1=len;
  for(remain=len,p=0;remain>0;){
    if(remain<MAX_MSG_SIZE){
      memcpy(session_msg->data1,mem+p,remain);
      session_msg->buf_size=remain;
      remain=0;
    }
    else{
      memcpy(session_msg->data1,mem+p,MAX_MSG_SIZE);
      remain-=MAX_MSG_SIZE;
      session_msg->buf_size=MAX_MSG_SIZE;
      p+=MAX_MSG_SIZE;
    }
    do{ // wait until the buffer data is cleared
      shm_notify(&session_msg->state,SERVER);
      if(shm_wait(&session_msg->state,SERVER,waiting_time)){
        session_msg->is_end=8;
        shm_notify(&session_msg->state,SERVER);
        return -1;
      }
      if(session_msg->is_end)
        return -1;
    }while(session_msg->buf_size>0);
  }
...
int send_over_network()
{
  ...
  if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size, disable_shmsync)) goto HANDLE_RESPONSES;
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize, disable_shmsync);
    messages_sent++;
    ...
    if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size, disable_shmsync)) {
      goto HANDLE_RESPONSES;
    }
    //if(prev_buf_size==response_buf_size)
    //  printf("%d poll timeout in network mode\n",child_pid);
...
HANDLE_RESPONSES:
  ...
  net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size, disable_shmsync);
...
int send_over_shm()
{
  ...
  *is_M2 = 0;
  ...
  //retrieve early server response if needed
  if (shm_read(sockfd,&response_buf, &response_buf_size)>0){
    likely_buggy = 1;
    //WARNF("PID %d - Responses from server are empty. You may prolong futex's wait time(default: 50ms) through '-L'.\n",child_pid);
    goto HANDLE_RESPONSES;
  }
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    *is_M2 = *is_M2 || messages_sent==M2_start_region_ID;
    n = shm_write(sockfd, kl_val(it)->mdata, kl_val(it)->msize);
    messages_sent++;
  ...
  while(!*is_M2){ // reading remaining messages from the server
    is_error=shm_read(sockfd,&response_buf,&response_buf_size);
    if(is_error>0){
      likely_buggy = 1;
      goto HANDLE_RESPONSES;
    }
  ...
  HANDLE_RESPONSES:
  //printf("%d enter HANDLE RESPONSES msg len:%d\n",child_pid,session_msg->len2);
  if(session_msg->len2>0){
    response_buf=(char *)ck_realloc(response_buf,response_buf_size+session_msg->len2);
    memcpy(response_buf+response_buf_size,session_msg->data2,session_msg->len2);
    response_buf_size+=session_msg->len2;
    session_msg->len2=0;
  }
  if(sockfd>=0){
    close(sockfd);
    session_msg->is_end=7;
    shm_notify(&session_msg->state,SERVER);
  }
  if (messages_sent > 0 && response_bytes != NULL) {
    response_bytes[messages_sent - 1] = response_buf_size;
  }
  //printf("%d session ends %d\n",child_pid,session_msg->is_end);
...
static void remove_shm(void) {
  ...
  shmctl(shm_netport_id, IPC_RMID, NULL);
...
EXP_ST void setup_shm(void) {
  ...
  u8* shm_str,*shm_msg_str,*shm_netport_str,*shm_check_str;
  ...
  shm_msg_id = shmget(IPC_PRIVATE, sizeof(msg), IPC_CREAT | IPC_EXCL | 0600);
  shm_netport_id = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | IPC_EXCL | 0600);
  shm_check_id = shmget(IPC_PRIVATE, sizeof(int)*9, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id < 0 || shm_msg_id<0 || shm_netport_id<0 || shm_check_id<0) PFATAL("shmget() failed");

  atexit(remove_shm);

  shm_str = alloc_printf("%d", shm_id);
  shm_msg_str = alloc_printf("%d", shm_msg_id);
  shm_netport_str = alloc_printf("%d", shm_netport_id);
  shm_check_str =alloc_printf("%d",shm_check_id);
  ...
  if (!dumb_mode){
    ...
    setenv(SHM_ENV_MSG_VAR, shm_msg_str, 1);
    setenv(SHM_ENV_NETPORT_VAR, shm_netport_str, 1);
    setenv(SHM_ENV_CHECK_VAR, shm_check_str, 1);
  }

  ck_free(shm_str);
  ck_free(shm_msg_str);
  ck_free(shm_netport_str);
  ck_free(shm_check_str);

  trace_bits = shmat(shm_id, NULL, 0);
  session_msg = (msg *)shmat(shm_msg_id, NULL ,0);
  netport_ptr = (int *)shmat(shm_netport_id, NULL, 0);
  sign_ptr = (int *)shmat(shm_check_id, NULL, 0);
  
  if (!trace_bits || !session_msg || !netport_ptr || !sign_ptr) PFATAL("shmat() failed");
  *netport_ptr = net_port;

  memset(sign_ptr,0,9*sizeof(int));
  sign_ptr[5]=(int)terminate_child;
  sign_ptr[6]=disable_shm;
  sign_ptr[7]=disable_shmsync;
  is_M2=&sign_ptr[8];
}
...
static void init_session(){
  shm_notify(&session_msg->state,SERVER);
  session_msg->is_end=0;
  session_msg->is_init=0;
  session_msg->len1=0;
  session_msg->len2=0;
  session_msg->buf_size=0;
  session_msg->mx.avail=1;
  session_msg->mx.waiters=0;
}
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    if (use_net) send_over_shm();
    if (waitpid(child_pid, &status, 0) <= 0) PFATAL("waitpid() failed");

  } else {
    if (use_net){
      if(!disable_shm && !sign_ptr[4])
        send_over_shm();
      else
        send_over_network();
    }
  ...
  if (!WIFSTOPPED(status)){
    while(1) {
      status2 = kill(child_pid, 0);
      if ((status2 != 0) && (errno == ESRCH)) break;
    }
    child_pid = 0;
  }
...
static u8 run_target_over_net(){
  u8 fault;
  int old_sync=disable_shmsync,old_term=terminate_child;
  
  sign_ptr[2]=UNINITIALIZED;
  sign_ptr[4]=1; // disable shared memory
  disable_shmsync=1; // disable synchronization based on shared memory
  terminate_child=1;
  fault=run_target(use_argv,exec_tmout);
  sign_ptr[4]=0;
  disable_shmsync=old_sync;
  terminate_child=old_term;
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if(bogus_crash_check && !disable_shm && fault==FAULT_CRASH) fault=run_target_over_net();
...
abandon_entry:
  ...
  if (!stop_soon && !queue_cur->cal_failed && !queue_cur->was_fuzzed) {
    ...
    //&& was_fuzzed_map[get_state_index(target_state_id)][queue_cur->index]==0) pending_favored--; // in case that pending_favored becomes negative
...
static void handle_timeout(int sig) {
  char cmd[64];
  //printf("timeoutttttttttttttttttttttttttttttttttttttttttttttttttttttttt %d\n",session_msg->state);
  if (child_pid > 0) {
    child_timed_out = 1;
    sprintf(cmd,"pkill -9 -P %d",child_pid); // kill all of the children of the server process, otherwise the children may still be alive after the server process is dead
    system(cmd);
    kill(child_pid, SIGKILL);
    session_msg->is_end=3;
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, msgs_per_sec\n");
                     /* ignore errors */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:L:bIY")) > 0)
    ...
    switch (opt) {
      ...
      case 'N': /* Network configuration */
        ...
        setenv(ENV_PROTOCOL_TYPE,net_protocol?"UDP":"TCP",1);
        use_net = 1;
        break;
      ...
      case 'P': /* protocol to be tested */
        ...
        } else if (!strcmp(optarg, "RESP")) {
          extract_requests = &extract_requests_resp;
          extract_response_codes = &extract_response_codes_resp;
        } else {
      ...
      case 'L':
	//Normally, we do not need a waiting_time because the synchronizer can schedule messages. However, the server may freeze regardless of incoming messages. For instance, the live555 server stucks in the loop of "while (sessionId == 0 || lookupClientSession(sessionIdStr) != NULL);" due to the patch of "sessionId = (u_int32_t)8888;"
        if(waiting_time != WAITING_TIME) FATAL("Multiple -L options not supported");
        waiting_time = atoi(optarg);
        break;
      
      case 'b':
        if(bogus_crash_check) FATAL("Multiple -b options not supported");
        bogus_crash_check = 1;
        break;
      
      case 'I':  // disable shared memory
      	if(disable_shm) FATAL("Multiple -I options not supported");
	      disable_shm = 1;
	      break;
      
      case 'Y':  // disable synchronization based on shared memory
        if(disable_shmsync) FATAL("Multiple -Y options not supported");
        disable_shmsync = 1;
        break;
  ...
  if(disable_shmsync && !disable_shm) FATAL("Cannot set -Y without -I");
  ...
  if (state_aware_mode) {
    ...
    while (1) {
      ...
      //if(selected_seed)
      //  printf("select idx:%d name:%s\n",selected_seed->index,selected_seed->fname);
